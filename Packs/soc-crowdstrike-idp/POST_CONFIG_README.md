# SOC CrowdStrike Falcon IDP — Post-Installation Steps
...
3. Find **`SOC CrowdStrike Falcon - IDP All Alerts`** → right-click → **Enable**

> The rule ships `is_enabled: true`; if your tenant already imported it enabled, skip this step.
> If a native CrowdStrike marketplace rule for IDP is enabled, disable it so detections
> don't double-alert.
...
Confirm IDP detections are reshaping correctly:

```
dataset = crowdstrike_falcon_event_raw
| filter product = "idp"
| fields user_name, actor_effective_username, alert_name, originalalertsource, src_account_name, location_country_code
| limit 10
```
